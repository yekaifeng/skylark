...
## 使用Docker Linux Bridge驱动
![Deply Logo](doc/skylark-deploy-bridge.png?raw=true "Deploy Logo")
...
### 五.所有宿主机都要跑一次创建网络mynet(Linux Bridge驱动)
...
## 使用Docker MacVlan驱动
![Deply Logo](doc/skylark-deploy-macvlan.png?raw=true "Deploy Logo")
...
### 一. 配置Bond0 VLAN子设备Bond0.52（如有必要）,重启机器使配置生效
以下面为例,bond0上配了vlan 52
    ...
    # cat /etc/sysconfig/network-scripts/ifcfg-bond0.52
    VLAN=yes
    DEVICE=bond0.52
    NAME=bond0.52
    PHYSDEV=bond0
...
### 五.所有宿主机都要跑一次创建网络mynet(MacVLan驱动)
    ...
    --opt=com.docker.network.driver.mtu=1500 \
    --ipam-driver=skylark \
    --subnet=10.100.85.0/24 \
    --gateway=10.100.85.1 \
    -o parent=bond0.85 \
    mynet

    # --gateway 为所有容器的统一gateway

- Docker从v1.12 开始支持macvlan
